...
warning_like { Bio::Path::Find::Lane::StatusFile->new( status_file => file( qw( t data 09_lane_status_file bad.txt ) ) ) }
  qr/not a valid status file/,
  'warning with bad status file';

my $s;
lives_ok { $s = Bio::Path::Find::Lane::StatusFile->new( status_file => file( qw( t data 09_lane_status_file good.txt ) ) ) }
...
is $s->config_file, file( qw( t data 09_lane_status_file new_pipeline.conf ) ), 'config file is correct';
