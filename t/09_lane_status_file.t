...
throws_ok { Bio::Path::Find::LaneStatusFile->new( status_file => file('non-existent-file') ) }
  qr/can't find status file/,
  'exception when status file is not found';

warning_like { Bio::Path::Find::LaneStatusFile->new( status_file => file('t/data/09_lane_status_file/bad.txt') ) }
  qr/not a valid status file/,
  'warning with bad status file';

my $s;
lives_ok { $s = Bio::Path::Find::LaneStatusFile->new( status_file => file('t/data/09_lane_status_file/good.txt') ) }
...
lives_ok { $s = Bio::Path::Find::LaneStatusFile->new( status_file => file('t/data/09_lane_status_file/missing_config_file.txt') ) }
...
lives_ok { $s = Bio::Path::Find::LaneStatusFile->new( status_file => file('t/data/09_lane_status_file/unknown_pipeline_name.txt') ) }
