...
applied to the L<Bio::Path::Find::Lane> objects that we find. The Role is used
to adapt the C<Lane> for use with a particular "*find" script, e.g. C<pathfind>
or C<annotationfind>.

If C<script_role> is not supplied, a default value is generated by taking the
basename of the calling script and using that to look up the role in the
configuration. If there is no mapping for script name in the "C<script_roles>"
...
has 'script_role' => (
  ...
  isa     => Str,
  lazy    => 1,
  builder => '_build_script_role',
);

sub _build_script_role {
  my $self = shift;

  my $role = $self->config->{script_roles}->{ $self->{_script_name} };

  croak "ERROR: couldn't find a role for the current script ("
        . $self->{_script_name} . ')'
    if not defined $role;
